...
public class DicFhirClientConfig
{
	...
	@ProcessDocumentation(processNames = {
			"medizininformatik-initiativede_executeDataSharing" }, description = "The path for oidc discovery protocol", recommendation = "Change default value only if path differs from the oidc specification")
	@Value("${de.medizininformatik.initiative.data.sharing.dic.fhir.server.oauth2.discovery.path:/.well-known/openid-configuration}")
	...
	@ProcessDocumentation(processNames = {
			"medizininformatik-initiativede_executeDataSharing" }, description = "If set to true, OIDC validation will only log a warning and not throw an illegal state exception")
	@Value("${de.medizininformatik.initiative.data.sharing.dic.fhir.server.oauth2.discovery.validation.lenient:false}")
	...
	@ProcessDocumentation(processNames = {
			"medizininformatik-initiativede_executeDataSharing" }, description = "To enable an additional connection test on startup of the client reading Binary resources as stream, set to `true`")
	@Value("${de.medizininformatik.initiative.data.sharing.dic.fhir.server.binary.stream.client.connection.test.enabled:false}")
