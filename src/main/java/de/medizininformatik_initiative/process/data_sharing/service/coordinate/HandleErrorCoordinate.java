...
public class HandleErrorCoordinate implements ServiceTask
{
	...
	private void sendMail(ProcessPluginApi api, Variables variables)
	{
		...
		String error = variables.getString(
				ConstantsDataSharing.BPMN_EXECUTION_VARIABLE_DATA_SHARING_COORDINATE_ERROR_MESSAGE);
		...
		String message = "Could not start consolidate data-sets in process '"
				+ ConstantsDataSharing.PROCESS_NAME_FULL_COORDINATE_DATA_SHARING + "' and Task '" + api.getTaskHelper()
				.getLocalVersionlessAbsoluteUrl(task) + "' at DMS '" + dmsIdentifier
